...
/// <see cref="MemoryStreamSlim"/> instances using the Brotli compression algorithm.
/// </summary>
public static class MemoryStreamBrotli
{
    ...
    private static void Compress(byte[] source, MemoryStreamSlim destination,
        MemoryStreamBrotliOptions? compressionOptions)
    {
        // Create the compression stream and write the source data to it.
#if NET9_0_OR_GREATER
        using (BrotliStream compressionStream = (compressionOptions is null) ?
                   new BrotliStream(destination, MemoryStreamBrotliOptions.DefaultCompressionLevel, true) :
                   (compressionOptions.Quality.HasValue ?
                       new BrotliStream(destination, new BrotliCompressionOptions { Quality = compressionOptions.Quality.Value }, true) :
                       new BrotliStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        using (BrotliStream compressionStream = new(destination,
    ...
    private static async Task CompressAsync(Stream source, MemoryStreamSlim destination,
        MemoryStreamBrotliOptions? compressionOptions, CancellationToken cancellationToken)
    {
        long startPosition = source.Position;
        source.Position = 0;
        // Create the compression stream and write the source data to it.
#if NET9_0_OR_GREATER
        await using (BrotliStream compressionStream = (compressionOptions is null) ?
                         new BrotliStream(destination, MemoryStreamBrotliOptions.DefaultCompressionLevel, true) :
                         (compressionOptions.Quality.HasValue ?
                             new BrotliStream(destination, new BrotliCompressionOptions { Quality = compressionOptions.Quality.Value }, true) :
                             new BrotliStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        await using (BrotliStream compressionStream = new(destination,
            compressionOptions?.CompressionLevel ?? MemoryStreamZLibBaseOptions.DefaultCompressionLevel, true))
#endif
        {
            // Do the actual compression operation
    ...
    private static void Decompress(Stream source, MemoryStreamSlim destination)
    {
        ...
        using (BrotliStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        source.Position = 0;
        // Create the decompression stream and write the decompressed data to the destination stream.
        await using (BrotliStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
            // Do the actual decompression operation
    ...
    /// </returns>
    public static MemoryStreamSlim Compress(byte[] source,
        Func<MemoryStreamBrotliOptions, MemoryStreamBrotliOptions> optionsSetup)
    {
        ArgumentNullException.ThrowIfNull(source, nameof(source));
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamBrotliOptions compressionOptions = optionsSetup(new());
        if (source.Length == 0)
            return MemoryStreamSlim.Create(compressionOptions.NewStreamOptions);
    ...
    public static MemoryStreamSlim Compress<TState>(byte[] source,
        Func<MemoryStreamBrotliOptions, TState, MemoryStreamBrotliOptions> optionsSetup, TState setupState)
    {
        ArgumentNullException.ThrowIfNull(source, nameof(source));
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamBrotliOptions compressionOptions = optionsSetup(new(), setupState);
        if (source.Length == 0)
            return MemoryStreamSlim.Create(compressionOptions.NewStreamOptions);
    ...
    /// </returns>
    public static async Task<MemoryStreamSlim> CompressAsync(Stream source,
        Func<MemoryStreamBrotliOptions, MemoryStreamBrotliOptions> optionsSetup, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamCompression.ValidateCompression(source);
        MemoryStreamBrotliOptions compressionOptions = optionsSetup(new());
