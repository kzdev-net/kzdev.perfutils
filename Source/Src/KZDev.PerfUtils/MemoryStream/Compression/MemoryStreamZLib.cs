...
/// <see cref="MemoryStreamSlim"/> instances using the ZLib compression algorithm.
/// </summary>
public static class MemoryStreamZLib
{
    ...
    private static void Compress(byte[] source, MemoryStreamSlim destination,
        MemoryStreamZLibOptions? compressionOptions)
    {
        // Create the compression stream and write the source data to it.
#if NET9_0_OR_GREATER
        using (ZLibStream compressionStream = (compressionOptions is null) ?
            ...
            new ZLibStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        using (ZLibStream compressionStream = new(destination,
    ...
    private static async Task CompressAsync(Stream source, MemoryStreamSlim destination,
        MemoryStreamZLibOptions? compressionOptions, CancellationToken cancellationToken)
    {
        long startPosition = source.Position;
        source.Position = 0;
        // Create the compression stream and write the source data to it.
#if NET9_0_OR_GREATER
        await using (ZLibStream compressionStream = (compressionOptions is null) ?
            new(destination, MemoryStreamZLibBaseOptions.DefaultCompressionLevel, true) :
            (compressionOptions.CompressionStrategy.HasValue ?
                new(destination, new ZLibCompressionOptions { CompressionStrategy = compressionOptions.CompressionStrategy.Value }, true) :
                new ZLibStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        await using (ZLibStream compressionStream = new(destination,
            compressionOptions?.CompressionLevel ?? MemoryStreamZLibBaseOptions.DefaultCompressionLevel, true))
#endif
        {
            // Do the actual compression operation
    ...
    private static void Decompress(Stream source, MemoryStreamSlim destination)
    {
        ...
        using (ZLibStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        source.Position = 0;
        // Create the decompression stream and write the decompressed data to the destination stream.
        await using (ZLibStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
            // Do the actual decompression operation
    ...
    /// </returns>
    public static MemoryStreamSlim Compress(byte[] source,
        Func<MemoryStreamZLibOptions, MemoryStreamZLibOptions> optionsSetup)
    {
        ArgumentNullException.ThrowIfNull(source, nameof(source));
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamZLibOptions compressionOptions = optionsSetup(new());
        if (source.Length == 0)
            return MemoryStreamSlim.Create(compressionOptions.NewStreamOptions);
    ...
    public static MemoryStreamSlim Compress<TState>(byte[] source,
        Func<MemoryStreamZLibOptions, TState, MemoryStreamZLibOptions> optionsSetup, TState setupState)
    {
        ArgumentNullException.ThrowIfNull(source, nameof(source));
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamZLibOptions compressionOptions = optionsSetup(new(), setupState);
        if (source.Length == 0)
            return MemoryStreamSlim.Create(compressionOptions.NewStreamOptions);
    ...
    /// </returns>
    public static async Task<MemoryStreamSlim> CompressAsync(Stream source,
        Func<MemoryStreamZLibOptions, MemoryStreamZLibOptions> optionsSetup, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(optionsSetup, nameof(optionsSetup));
        MemoryStreamCompression.ValidateCompression(source);
        MemoryStreamZLibOptions compressionOptions = optionsSetup(new());
