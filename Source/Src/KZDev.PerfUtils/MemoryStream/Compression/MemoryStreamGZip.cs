...
/// <see cref="MemoryStreamSlim"/> instances using the GZip compression algorithm.
/// </summary>
public static class MemoryStreamGZip
{
        ...
        MemoryStreamGZipOptions? compressionOptions)
    {
        ...
        using (GZipStream compressionStream = (compressionOptions is null) ?
            ...
            new GZipStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        using (GZipStream compressionStream = new(destination,
        ...
        MemoryStreamGZipOptions? compressionOptions, CancellationToken cancellationToken)
    {
        ...
        // Create the compression stream and write the source data to it.
#if NET9_0_OR_GREATER
        await using (GZipStream compressionStream = (compressionOptions is null) ?
            new(destination, MemoryStreamZLibBaseOptions.DefaultCompressionLevel, true) :
            (compressionOptions.CompressionStrategy.HasValue ?
                new(destination, new ZLibCompressionOptions { CompressionStrategy = compressionOptions.CompressionStrategy.Value }, true) :
                new GZipStream(destination, compressionOptions.CompressionLevel!.Value, true)))
#else
        await using (GZipStream compressionStream = new(destination,
            compressionOptions?.CompressionLevel ?? MemoryStreamZLibBaseOptions.DefaultCompressionLevel, true))
#endif
        {
            // Do the actual compression operation
    ...
    private static void Decompress(Stream source, MemoryStreamSlim destination)
    {
        ...
        using (GZipStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        source.Position = 0;
        // Create the decompression stream and write the decompressed data to the destination stream.
        await using (GZipStream compressionStream = new(source, CompressionMode.Decompress, true))
        {
            // Do the actual decompression operation
